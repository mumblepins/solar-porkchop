...
(function() {
  ...
  this.lambert = function(mu, pos1, pos2, dt, maxRevs, prograde) {
    var N, angleParameter, c, deltaPos, ftau, fy, i, invSqrtM, invSqrtN, m, minimumEnergyNormalizedTime, minimumNormalizedTime, n, normalizedTime, parabolicNormalizedTime, phix, phiy, pushSolution, r1, r2, ref, solutions, sqrtMu, transferAngle, x, x1, x2, xMT, y;
    if (maxRevs == null) {
      maxRevs = 0;
    }
    if (prograde == null) {
    ...
    if (relativeError(normalizedTime, parabolicNormalizedTime) < 1e-6) {
      ...
      for (N = i = 0, ref = maxRevs; 0 <= ref ? i <= ref : i >= ref; N = 0 <= ref ? ++i : --i) {
        if (N > 0 && N === maxRevs) {
          phix = function(x) {
            var g;
            g = Math.sqrt(1 - x * x);
